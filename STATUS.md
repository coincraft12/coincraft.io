...
## 인프런 벤치마킹 기반 기능 To-Do (2026-04-17 조사)

### 🔴 필수 (단기 — 다음 Phase에 포함)
- [ ] **로드맵 (학습 경로)**: 강의 조합으로 커리큘럼 제시 (예: "WEB3 입문 → Solidity → Custody") — 강좌 페이지 상단 배너 + 별도 `/roadmaps` 페이지
- [ ] **Q&A 게시판**: 강좌별 수강생-강사 질문/답변 스레드 (`/courses/:slug/qna`) — 수강생 retention에 직접 영향
- [ ] **스킬 태그**: 강좌에 기술 태그 (Solidity, Web3.js, DeFi 등) 부착 → 필터/검색 강화
- [ ] **쿠폰/할인 코드**: 마케팅용 쿠폰 생성·적용 (결제 페이지 쿠폰 입력란)
- [ ] **강사 수익 대시보드**: 강사 포털에 매출·정산 현황 표시 (`/instructor/revenue`)
- [ ] **수강생 현황**: 강사 포털에서 강좌별 수강생 목록·진도율·완료 여부 확인 (`/instructor/courses/[id]/students`)

### 🟡 고려 (중기 — Phase 6~7)
- [ ] **스터디 그룹**: 수강생 자율 그룹 결성, 강좌 연동 (커뮤니티 활성화)
- [ ] **AI 자막/노트**: 레슨 영상 자동 자막 생성 (OpenAI Whisper) + 타임스탬프 노트
- [ ] **멘토링 예약**: 강사 1:1 상담 일정 예약 (캘린더 연동)
- [ ] **UGC 블로그/노트**: 수강생 학습 노트 공개 공유 (SEO + 커뮤니티 효과)
- [ ] **강의 완료 공유**: 수료증 SNS 공유 버튼 (LinkedIn 공유 — 바이럴)

### ⚪ 불필요/보류
- 모바일 오프라인 저장 — 유료 CDN 비용 대비 효과 낮음
- B2B 기업 교육 패키지 — 현재 단계에선 오버스펙
- 포인트/캐시 적립 — PortOne 연동 충분한 시점에 재검토
