/**
 * 워드프레스 → 새 플랫폼 마이그레이션 이메일 일괄 발송 스크립트
 *
 * 실행:
 *   npx ts-node -r dotenv/config --project tsconfig.json scripts/send-migration-emails.ts
 *   (운영 DB 대상: DATABASE_URL=... npx ts-node ...)
 *
 * 동작:
 *   1. 비-OAuth 사용자 전체 조회
 *   2. 비밀번호를 임시 비밀번호(TEMP_PASSWORD)로 일괄 초기화
 *   3. 마이그레이션 안내 이메일 발송
 *
 * 주의:
 *   - 실제 운영 DB에 비밀번호를 변경하므로 반드시 Sharon 승인 후 실행
 *   - 이미 bcrypt 해시가 정상인 사용자도 포함해서 전체 초기화 (통일)
 */

import 'dotenv/config';
import { drizzle } from 'drizzle-orm/node-postgres';
import { Pool } from 'pg';
import { ne, like, or, notLike } from 'drizzle-orm';
...
async function sendMigrationEmail(
  ...
  if (!transporter) {
    console.log(`[stub] ${to} — 이메일 발송 스킵 (GMAIL 설정 없음)`);
    return;
  }
...
async function main() {
  ...
  // 비-OAuth 사용자 조회 (kakao_, wallet_ 접두사 제외)
  ...
  console.log(`bcrypt 해시 생성 완료 (rounds: ${SALT_ROUNDS})`);
