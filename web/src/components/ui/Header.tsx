...
const LOGO = 'https://coincraft.io/wp-content/uploads/2026/01/COINCRAFT_IO_헤더로고-복사.png'
...
const nav = [
  ...
  { label: 'ABOUT', href: '#about' },
  {
    ...
    href: '#',
    children: [
      { label: 'Web3', href: '/course-category/web3/' },
      { label: '온체인 데이터 분석', href: '/course-category/onchain-data/' },
    ],
  },
  {
    label: '검정',
    href: '/cert/',
    children: [
      { label: '검정 신청', href: '/cert/application/' },
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#1a1a2e] border-b border-white/10">
      <div className="max-w-cc mx-auto px-6 flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center gap-3">
          <a href="/account/?action=register" className="text-sm text-cc-muted hover:text-cc-text transition-colors">
            회원가입
          </a>
          <a href="/account/" className="cc-btn cc-btn-primary text-sm px-4 py-2">
            로그인
          </a>
        </div>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-[#1a1a2e] border-t border-white/10 px-6 py-4">
          ...
          <div className="flex gap-3 pt-3 border-t border-white/10 mt-2">
            <a href="/account/?action=register" className="text-sm text-cc-muted">회원가입</a>
            <a href="/account/" className="cc-btn cc-btn-primary text-sm px-4 py-2">로그인</a>
          </div>
        </div>
      )}
    </header>
